#!/usr/bin/env python3
"""
Модель 4: Logistic Regression + RuBERT (дообученный на тендерах)
Эмбеддинги от дообученного RuBERT + логистическая регрессия
"""
...
print("=" * 60)
print("📊 МОДЕЛЬ 4: LOGREG + RuBERT (ДООБУЧЕННЫЙ)")
print("=" * 60)
...
# Если модель ещё не дообучена — используй базовую с предупреждением
finetuned_model_path = "/root/VKR/модель_классификации/rubert_finetuned"  # ← измени при необходимости
...
y_pred = model_lr.predict(X_test_emb)
y_proba = model_lr.predict_proba(X_test_emb)[:, 1]
...
cv_scores = cross_val_score(model_lr, X_train_emb, y_train, cv=cv, scoring='f1')
...
plt.title('Confusion Matrix: LogReg + RuBERT (finetuned)')
plt.ylabel('True')
